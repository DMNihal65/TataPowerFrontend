...
const DocumentRetrieval = () => {
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://172.18.100.88:7000/get-files/', {
                  ...
                  cover={
                    <div className="h-40 flex items-center justify-center bg-gray-200">
                      {doc.file_name.toLowerCase().endsWith('.pdf') ? (
                        <Document file={`file://${doc.file_path}`}>
                          <Page pageNumber={1} width={150} />
                        </Document>
                      ) : (
                        <img
                          alt={doc.file_name}
                          src={`file://${doc.file_path}`}
          ...
          >
            {currentDocument && (
              currentDocument.file_name.toLowerCase().endsWith('.pdf') ? (
                <Document file={`file://${currentDocument.file_path}`}>
                  <Page pageNumber={1} width={800} />
                </Document>
              ) : (
                <img
                  alt={currentDocument.file_name}
                  src={`file://${currentDocument.file_path}`}
